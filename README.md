...
- [`direct_data_driven_mpc/utilities/controller/initial_data_generation.py`](direct_data_driven_mpc/utilities/controller/initial_data_generation.py): Provides functions for generating input-output data from LTI and nonlinear systems, which can be used as initial trajectories in Data-Driven MPC controller creation.
...
Custom functions are provided in [`direct_data_driven_mpc/utilities/visualization/`](direct_data_driven_mpc/utilities/visualization/) to display input-output data in static and animated plots. These functions use Matplotlib for visualization and FFmpeg for saving animations in various formats (e.g., GIF, MP4).
